...
function formElements(propValue, errors) {
    ...
    propValue.forEach(x => {
        if(!x.key) {
            errors.push({
                error: "Each object of formElements array must have defined 'key' property."
            })
        }
    })
...
function cancel(propValue, errors) {
    ...
    if(!propValue.color) {
        errors.push({
            isWarning: true,
            error: "Prop 'cancel' should have defined 'color' property. Using white..."
        })
    }
}
...
function submit(propValue, errors) {
    ...
    if(!propValue.color) {
        errors.push({
            isWarning: true,
            error: "Prop 'submit' should have defined 'color' property. Using white..."
        })
    }
}
